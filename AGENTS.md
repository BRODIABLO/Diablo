...
- **Topologie des dépôts Suite** : `C:\Workspaces\Diablo` porte l'incubation et l'intégration runtime; `C:\Workspaces\RuffnecKk-D2RLoader-Suite` est la source publique du produit; `C:\Workspaces\RuffnecKk-D2RLoader-Suite-Governance` porte les registres et allowlists privés. Résoudre ces chemins par `workspace-repositories.json` et ses variables d'environnement. Aucun dépôt autoritaire ne doit résider sous `analysis-cache/`, réservé aux caches, builds, preuves runtime, sauvegardes et staging jetable.
