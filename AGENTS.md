...
- **Crédit D2MOO** : dès qu’un plugin a nécessité des connaissances acquises grâce à D2MOO, créditer explicitement D2MOO dans le README du plugin. Conserver ce README avec la documentation du projet, hors du ZIP public strict limité à la DLL et à sa configuration.
...
- **Gate absolu pour chaque nouveau plugin** : avant toute implantation, utiliser le skill `d2rloader-plugin-incubation` et demander d’abord uniquement à Vincent : « Plugin autonome ou plugin à merger au PluginPack ? » Attendre sa réponse explicite avant de modifier code, configuration ou archive. Pour un plugin autonome permanent, ne demander ni catégorie, ni DLL propriétaire, ni clé de merge : conserver une DLL RuffnecKk hybride compatible et testée avec les cinq plugins du pack, avec son propre fichier de configuration indépendant en JSON ou en TOML lorsque TOML est plus convivial. Pour un plugin destiné au merge, proposer ensuite le propriétaire futur (`items`, `levels`, `misc`, `quests` ou `skills`) et la clé `categorie.nomFonctionnalite`, puis attendre leur confirmation avant l’implantation; conserver pendant l’incubation une DLL hybride et un JSON autonome compatible avec le futur bloc de `D2RPlugins.json`. Dans les deux voies, ne modifier, lier ni redistribuer aucune DLL d’eezstreet.
...
- `d2rloader-plugin-incubation` — choix autonome ou merge, audit de compatibilité PluginPack, configuration indépendante, crédits et ZIP;
