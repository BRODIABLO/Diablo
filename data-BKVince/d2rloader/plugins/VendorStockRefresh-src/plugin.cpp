...
using ruffneckk::vendor_stock_refresh::CenterBelow;
using ruffneckk::vendor_stock_refresh::HasUsableSize;
using ruffneckk::vendor_stock_refresh::NormalRefreshAction;
using ruffneckk::vendor_stock_refresh::UnionRect;
using ruffneckk::vendor_stock_refresh::VanillaNormalVendorAction;
using ruffneckk::vendor_stock_refresh::WidgetRect;
...
constexpr std::array<std::uint8_t, 32> ConfigureVendorPanelExpected{
...
bool ValidateRuntime() noexcept {
    return Matches(SendVendorRefreshRva, SendVendorRefreshExpected)
        && Matches(EntityActionRva, EntityActionExpected)
        && Matches(ConfigureVendorInteractionRva, ConfigureVendorInteractionExpected)
        && Matches(IsGamblingRva, IsGamblingExpected)
        && Matches(SendNineBytePacketRva, SendNineBytePacketExpected)
        && Matches(GetVendorChainEntryRva, GetVendorChainEntryExpected)
        && Matches(ConfigureVendorPanelRva, ConfigureVendorPanelExpected)
...
void __fastcall HookConfigureVendorInteraction(
    ...
    if (Settings.enabled
        ...
        && requestedMode == ruffneckk::vendor_stock_refresh::NormalVendorMode) {
        __try {
...
auto Status(D2R::Game::Client*, const D2RL::ConsoleCommandContext* command, void*) noexcept
    ...
    std::snprintf(
        message,
        sizeof(message),
        "VendorStockRefresh 0.1.5: enabled=%s; JSON config=%s; placed=%llu; placementFailures=%llu; sent=%llu; received=%llu; armed=%llu; rejected=%llu.",
...
D2RL_PLUGIN_EXPORT auto D2RLoaderLoadPlugin(const D2RL::PluginContext* context) noexcept -> bool {
    ...
    if (Settings.enabled) {
        ...
        if (!context->InstallInlineHook(
            ...
            context->LogError("VendorStockRefresh: vendor-panel configuration hook failed.");
            return false;
        }
        if (!context->InstallInlineHook(
        ...
        if (!context->InstallInlineHook(
            ...
            context->LogError("VendorStockRefresh: server entity-action hook failed.");
            return false;
        }
        if (!context->InstallInlineHook(
    ...
    const auto message = std::string("VendorStockRefresh 0.1.5 ") + state
        + " for D2R 3.2.92777; marked normal-vendor refresh uses opcode 0x38"
        + " and repositions the native button from the runtime gold anchor"
