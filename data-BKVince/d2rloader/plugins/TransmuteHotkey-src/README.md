...
- Default hotkey: `CTRL+SHIFT+T` with exact modifier matching.
- One activation is queued per physical key press; Windows repeats are ignored.
...
Printable keys require `CTRL` or `ALT`. Supported names are `A-Z`, `0-9`,
`F1-F24`, `SPACE`, `TAB`, `INSERT`, `DELETE`, `HOME`, `END`, `PAGEUP`, and
`PAGEDOWN`, combined with `CTRL`, `SHIFT`, or `ALT`.
