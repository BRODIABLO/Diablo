...
3. Exiger une DLL indépendante, hybride globale/mod-locale, avec sa propre version, ses métadonnées et son archive.
4. Prouver qu'au moins un réglage moddeur réel, distinct de la présence de la DLL, existe avant d'ajouter un fichier de configuration. Un booléen `enabled` seul ne justifie pas un fichier : sans réglage réel, la DLL est active par sa présence et se distribue seule. Lorsqu'une configuration est justifiée, la garder indépendante de `D2RPlugins.json`, choisir JSON ou TOML selon la convivialité réelle pour le moddeur et rédiger son contenu et ses commentaires en anglais.
5. Autoriser un compagnon runtime appartenant au plugin seulement lorsqu'il est indispensable au comportement retenu. Le consigner dans le registre et l'allowlist, le placer sous `plugins/` dans le ZIP et prouver que son absence échoue proprement.
...
4. Si une configuration est justifiée, la rechercher d'abord dans le mod actif puis dans le dossier global du jeu et refuser explicitement un fichier présent mais invalide. Sans réglage réel, ne conserver aucun parseur, dépendance, chemin ni artefact de configuration.
5. Conserver une version de composant indépendante. Lorsqu'un JSON ou TOML dédié existe, ne jamais déplacer sa configuration dans `D2RPlugins.json`.
...
4. Créer ou actualiser le README du plugin et le déposer à côté du ZIP dans le dossier de livraison. Produire le ZIP généré par l'agent avec la DLL autonome, son éventuel compagnon runtime indispensable et gouverné sous `plugins/`, puis, seulement si le contrat confirmé en a réellement besoin, son fichier de configuration indépendant JSON ou TOML sous `config/`. Exclure README, sources, symboles, logs et fichiers de preuve; Vincent relit et modifie humainement le README avant de l'ajouter lui-même au ZIP final.
...
8. Dès que le composant qualifié est retenu pour la Suite ou une prochaine release, appliquer automatiquement le skill `d2rloader-suite-promotion`. Ne pas attendre que Vincent le nomme : ce gate bascule la source autoritaire de `workspace:addons/...` vers `suite:...` avant tout état `package-ready`.
