...
auto Status(D2R::Game::Client*, const D2RL::ConsoleCommandContext* command, void*) noexcept
    ...
    std::snprintf(
        message,
        sizeof(message),
        "RemoteStash 0.2.25: layoutBindings=%llu; bindingFailures=%llu; "
...
D2RL_PLUGIN_EXPORT auto D2RLoaderLoadPlugin(const D2RL::PluginContext* context) noexcept
    ...
    context->LogInfo(
        UsingUiMessageBroker.load(std::memory_order_acquire)
            ? "RemoteStash 0.2.25 inter-mod active for D2R 3.2.92777; the mod-owned desktop "
              "button uses the shared PluginPack UI dispatcher and remote stash sessions "
              "bypass native proximity checks."
            : "RemoteStash 0.2.25 inter-mod active for D2R 3.2.92777; the mod-owned desktop "
              "button uses the standalone UI dispatcher and remote stash sessions bypass "
              "native proximity checks."
    );
    return true;
}
