...
constexpr std::array LocaleLabelTable{
    ...
    LocaleLabels{"防禦：%d", "最大鑲孔數：%d", "基礎防禦：%d"},
    LocaleLabels{"Verteidigung: %d", "Maximale Sockel: %d", "Grundverteidigung: %d"},
    LocaleLabels{"Defensa: %d", "Engarces máximos: %d", "Defensa base: %d"},
    LocaleLabels{"Défense : %d", "Châsses maximales : %d", "Défense de base : %d"},
    LocaleLabels{"Difesa: %d", "Castoni massimi: %d", "Difesa base: %d"},
    LocaleLabels{"방어력: %d", "최대 홈: %d", "기본 방어력: %d"},
    LocaleLabels{"Obrona: %d", "Maksymalna liczba gniazd: %d", "Bazowa obrona: %d"},
    // esMX shares the same built-in labels as esES.
    LocaleLabels{"Defensa: %d", "Engarces máximos: %d", "Defensa base: %d"},
    LocaleLabels{"防御力: %d", "最大ソケット数: %d", "基本防御力: %d"},
    LocaleLabels{"Defesa: %d", "Soquetes máximos: %d", "Defesa base: %d"},
    LocaleLabels{"Защита: %d", "Максимум гнезд: %d", "Базовая защита: %d"},
    LocaleLabels{"防御: %d", "最大镶孔数：%d", "基础防御：%d"},
};
