...
bool LoadSettings() noexcept {
    const auto modDirectory = Context && Context->modDirectory
        ? std::filesystem::path(Context->modDirectory)
        : std::filesystem::path{};
    const auto pluginConfigPath = Context && Context->pluginConfigPath
        ? std::filesystem::path(Context->pluginConfigPath)
        : std::filesystem::path{};
    const auto candidates = ruffneckk::advanced_item_tooltips::ConfigCandidates(
        modDirectory, pluginConfigPath, ExecutableDirectory(), ConfigFileName);
    try {
        const auto loaded = ruffneckk::advanced_item_tooltips::LoadConfig(candidates);
        Settings = loaded.config;
        LoadedConfigPath = loaded.source;
        return true;
    } catch (const std::exception& exception) {
        if (Context) {
            const auto message = std::string("AdvancedItemTooltips: ") + exception.what() + ".";
