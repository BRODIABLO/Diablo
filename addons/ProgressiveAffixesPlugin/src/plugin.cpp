...
constexpr wchar_t ConfigFileName[] = L"ProgressiveAffixesPlugin.toml";
...
bool LoadConfig() noexcept {
    ...
    for (const auto& path : ConfigCandidates()) {
        ...
        try {
            ...
            if (!input.is_open()) throw std::runtime_error("configuration file cannot be opened");
            Settings = ParseConfig(input);
            LoadedConfigPath = path.string();
            return true;
        } catch (const std::exception& exception) {
            const auto message = std::string("ProgressiveAffixesPlugin: invalid ")
...
bool RefreshTypeCache(const void* item) noexcept {
    ...
    if (!valid) {
        ...
        if (!RuntimeConfigErrorLogged.test_and_set(std::memory_order_relaxed)) {
            Context->LogError(
                "ProgressiveAffixesPlugin: one or more configured item type codes "
...
bool CreateRelays() noexcept {
    ...
    if (!WritePreservingArgumentBridge(
        ...
        Context->LogError("ProgressiveAffixesPlugin: could not build relay code.");
        return false;
    }
    DWORD oldProtection{};
    if (!VirtualProtect(
            ...
            PAGE_EXECUTE_READ,
            &oldProtection)) {
        Context->LogError("ProgressiveAffixesPlugin: could not protect relay code.");
        return false;
    }
...
bool ValidateSignatures() noexcept {
    ...
    constexpr std::array<std::uint8_t, 24> checkType{
    ...
    return Matches(GeneralSeedRollRva, generalRoll)
        ...
        && Matches(CheckItemTypeRva, checkType)
...
bool InstallPatches() noexcept {
    ...
    std::array<std::uint8_t, rareSelectionExpected.size()> rareSelection{};
    rareSelection.fill(0x90);
    ...
    constexpr std::array<std::uint8_t, 3> moveCount{0x41,0x8B,0xE0};
    constexpr std::array<std::uint8_t, 3> restoreSeed{0x4C,0x8B,0xCB};
    std::memcpy(rareSelection.data() + 5, moveCount.data(), moveCount.size());
    std::memcpy(rareSelection.data() + 8, restoreSeed.data(), restoreSeed.size());
    if (!EncodeRel32(
            rareSelection.data() + 11,
            0xE9,
            rareSite + 11,
...
D2RL_PLUGIN_EXPORT auto D2RLoaderLoadPlugin(
    ...
    if (context->modDataVersionBuild != 0
        ...
        Context->LogError(
            "ProgressiveAffixesPlugin: supports only D2R 3.2 build 92777.");
        return false;
    }
    ...
    if (!ValidateSignatures()) {
        Context->LogError(
            "ProgressiveAffixesPlugin: build signature mismatch or conflicting affix owner; plugin refused.");
        return false;
    }
    if (!CreateRelays() || !InstallPatches()) {
        Context->LogError(
            "ProgressiveAffixesPlugin: affix-selection relays or patches could not be installed.");
        return false;
    }
