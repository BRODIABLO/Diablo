...
export const REVIEW_REQUIRED_CATEGORIES = Object.freeze(['PD2_DELETED', 'PD2_MODIFIED', ...NEW_AFFIX_CATEGORIES]);
...
export function entryReviewState(entry, selection = {}) {
  if (!REVIEW_REQUIRED_CATEGORIES.includes(entry.category) || entry.deferred) {
    return { required: false, complete: true, reasons: [] };
  }
  if (isNewAffix(entry)) {
    const lineDecision = selection.lineDecision ?? '';
    if (!lineDecision) return { required: true, complete: false, reasons: ['line decision missing'] };
    if (lineDecision === 'EXCLUDE_PD2_AFFIX') return { required: true, complete: true, reasons: [] };
    if (lineDecision === 'DISCUSS') return { required: true, complete: false, reasons: ['line is marked for discussion'] };
    if (!['IMPORT_PD2_AFFIX', 'IMPORT_CUSTOMIZED'].includes(lineDecision)) {
      return { required: true, complete: false, reasons: [`unknown line decision ${lineDecision}`] };
    }
  }
  const reasons = [];
  if (isNewAffix(entry) && selection.lineDecision === 'IMPORT_CUSTOMIZED'
    && !Object.values(selection.fields ?? {}).some((choice) => choice?.decision === 'CUSTOM')) {
    reasons.push('IMPORT_CUSTOMIZED requires at least one CUSTOM field');
  }
  ...
  return { required: true, complete: reasons.length === 0, reasons };
}
...
export function validateDecisionEnvelope(report, payload) {
  ...
  for (const key of ['sourceHashes', 'targetBaselineHashes']) {
