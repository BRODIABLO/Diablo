...
function skillReviewBrowserApplication() {
  ...
  function proofStatusesOf(skill) {
    const statuses = [];
    statuses.push(
      ...asArray(skill.evidence?.overall),
      ...asArray(skill.evidence?.statuses),
      ...asArray(skill.evidence?.status),
      ...asArray(skill.evidence?.proofStatus),
      ...asArray(skill.proofStatus),
    );
  ...
  function renderBaselineNotice() {
    ...
    const historical = coverage.historicalBaseline || {};
    const current = coverage.currentBaseline || {};
    return Number.isInteger(current.bkvinceRows) && Number.isInteger(current.collisions)
      && Number.isInteger(historical.bkvinceRows) && Number.isInteger(historical.collisions)
      ? '<p class="result-summary baseline-drift"><strong>Baseline courante (HEAD) :</strong> ' + escapeHtml(current.bkvinceRows) + ' lignes BKVince · ' + escapeHtml(current.collisions) + ' collisions · prochain ordinal ' + escapeHtml(coverage.nextAppendOrdinal) + '. '
        + '<span><strong>Audit historique du 8 août :</strong> ' + escapeHtml(historical.bkvinceRows) + ' lignes · ' + escapeHtml(historical.collisions) + ' collisions · prochain ordinal ' + escapeHtml(historical.bkvinceRows) + '.</span></p>'
      : '';
  }
  ...
  function chip(value, kind) {
    const label = kind === 'proof' ? String(value) : friendly(value);
    return '<span class="chip ' + escapeAttribute(kind || '') + '">' + escapeHtml(label) + '</span>';
  }
...
function compressedOracleBootstrap() {
  ...
  async function inflateOracle(base64) {
    if (typeof DecompressionStream !== 'function') {
      throw new Error('Ce navigateur ne fournit pas DecompressionStream("gzip"), requis pour ouvrir cet artefact autonome. Utilisez une version récente de Chromium, Edge ou Firefox.');
    }
  ...
  function fatal(error) {
    ...
    const target = document.querySelector('#workbench') || document.body;
    target.innerHTML = '<main class="fatal"><h1>PD2 Skills Merge Workbench</h1><p>Impossible de décompresser l’oracle local.</p><pre></pre></main>';
    const details = target.querySelector('pre');
    if (details) details.textContent = message;
    globalThis.__PD2_SKILLS_WORKBENCH_ERROR__ = message;
  }

  globalThis.__PD2_SKILLS_WORKBENCH_READY__ = (async () => {
    try {
...
__PD2_SKILLS_APPLICATION__
      ...
      delete globalThis.__PD2_SKILLS_ORACLE_GZIP_BASE64__;
    }
  })();
}

export function buildSkillReviewHtml(report, browserRuntimeSource, options = {}) {
  ...
  if (typeof options.compressedOracleBase64 !== 'string' || !options.compressedOracleBase64.length) {
  ...
  return '<!doctype html>\n'
    ...
    + '<script>globalThis.__PD2_SKILLS_ORACLE_GZIP_BASE64__="' + options.compressedOracleBase64 + '";\n'
    + '/* Deterministic local gzip oracle; no network loader and no gameplay write path. */\n'
    + bootstrapSource + '\n</script></body></html>\n';
}
